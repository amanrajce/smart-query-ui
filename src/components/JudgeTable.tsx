  ...
  const MetricBar = ({ label, score }: { label: string, score: number }) => {
    ...
    return (
      <div className="flex items-center gap-3 mb-2">
        <span className="text-[10px] font-medium text-gray-500 w-20 uppercase tracking-tighter">
          {label}
        </span>
        <div className="flex-1 h-1 bg-[#2A2A2A] rounded-full overflow-hidden relative">
          <div 
            className="absolute top-0 left-0 h-full bg-gray-300 transition-all duration-700 ease-in-out" 
  ...
  return (
    <div className="mt-10 border border-[#3E3E3E] rounded-lg bg-[#1A1A1A] overflow-hidden">
      ...
      <div className="px-5 py-3 border-b border-[#3E3E3E] bg-[#212121] flex justify-between items-center">
        <h3 className="text-[12px] font-bold text-gray-400 uppercase tracking-[0.2em]">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-[#3E3E3E] bg-[#1C1C1C]">
              ...
              <th className="py-3 px-5 text-[10px] font-bold text-gray-500 uppercase tracking-widest w-48">Model Entity</th>
              <th className="py-3 px-5 text-[10px] font-bold text-gray-500 uppercase tracking-widest">Performance Metrics</th>
          ...
          <tbody className="divide-y divide-[#2A2A2A]">
            {evaluations.map((evalItem, index) => (
              <React.Fragment key={evalItem.modelName}>
                <tr className="hover:bg-[#222222] transition-colors">
                  ...
                  <td className="py-4 px-5">
                    <div className="grid grid-cols-1 gap-x-8">
                      <MetricBar label="Accuracy" score={evalItem.scores.accuracy} />
                      <MetricBar label="Clarity" score={evalItem.scores.clarity} />
                      <MetricBar label="Completeness" score={evalItem.scores.completeness} />
                    </div>
                  </td>
                ...
                {/* Reasoning Sub-row: Added for professional context without cluttering the main line */}
                <tr className="bg-[#1C1C1C]/50">
                  <td colSpan={4} className="px-5 py-2.5">
                    <p className="text-[11px] text-gray-500 leading-relaxed font-light italic max-w-3xl">
                      <span className="text-[9px] font-bold uppercase mr-2 not-italic text-gray-600">Analysis:</span>
                      {evalItem.reason}
                    </p>
                  </td>
                </tr>
